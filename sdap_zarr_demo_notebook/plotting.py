...
def base_map(bounds: dict = {}, padding: float = 2.5, show_roads=True) -> plt.axes:
    ...
    if isinstance(padding, (int, float)):
        lon_padding, lat_padding = padding, padding
    else:
        lon_padding, lat_padding = padding

    if bounds:
        bounds = (max(bounds['min_lon'] - lon_padding, -180),
                  min(bounds['max_lon'] + lon_padding, 180),
                  max(bounds['min_lat'] - lat_padding, -90),
                  min(bounds['max_lat'] + lat_padding, 90))
    else:
